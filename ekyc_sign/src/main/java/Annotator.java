...
public class Annotator {
	...
	public static String[] annotate(String payload) throws Exception {
		...
		for(i = 0; i < all_files.length; i++) {
			...
			Process p=Runtime.getRuntime().exec("scp /home/ubuntu/uploads/" + filename + " " + ReceiveRequest.NODE_MACHINE_ADDRESS +  ":/home/ubuntu/automated_deployment/indifi_source/arya/uploads/"); 
		...
		out.println(html);
		out.close();
		Process p=Runtime.getRuntime().exec("scp /home/ubuntu/uploads/" + rid + ".html" + " " + ReceiveRequest.NODE_MACHINE_ADDRESS + ":/home/ubuntu/automated_deployment/indifi_source/arya/uploads/");
		p.waitFor();
		Document document = new Document();
        ...
        document.close();
	Process p1=Runtime.getRuntime().exec("scp /home/ubuntu/uploads/" + rid + ".pdf" + " " + ReceiveRequest.NODE_MACHINE_ADDRESS +":/home/ubuntu/automated_deployment/indifi_source/arya/uploads/");
                p1.waitFor();
		String[] response = {"",rid};
		return response;
	}
